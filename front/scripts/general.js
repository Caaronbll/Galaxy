...
// Define initial money amount
let moneyAmount = 1000;

// Get the money box, subtract button, and price tag
const moneyBox = document.getElementById('moneyBox');
const subtractButton = document.getElementById('subtractButton');
const priceTag = document.getElementById('priceTag');

// Function to update the money box
function updateMoneyBox() {
    moneyBox.textContent = '' + moneyAmount;
}
...
subtractButton.addEventListener('click', function() {
    // Get the price of the selected planet
    const planetPrice = parseInt(priceTag.textContent.replace('', ''));

    // Check if there's enough money to buy the selected planet
    if (moneyAmount >= planetPrice) {
        // Subtract the price of the selected planet
        moneyAmount -= planetPrice;
        // Update the money box
        updateMoneyBox();
        // Optionally, you can perform additional actions here, like buying the planet
        console.log(`You've bought a planet for $${planetPrice}!`);
    } else {
        // If not enough money, show an alert
        alert("Not enough money to buy the selected planet!");
    }
});

// Initial update of the money box and price tag
updateMoneyBox();
updatePriceTag(1000); // Default price
